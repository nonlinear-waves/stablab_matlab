function prover(nu_left,nu_right,scale_R)
...
plot_it = 'off'; % plot the profile or not
...
% theta_R - the angle parameter for the right manifold
...
temp = 4-4*mu_boundaries-mu_boundaries.^2;
for j = 1:length(temp)
    if inf(temp(j)) < 0
        temp(j) = iv(0,sup(temp(j)));
    end
end
alpha_boundaries = sqrt(temp)./mu_boundaries;
