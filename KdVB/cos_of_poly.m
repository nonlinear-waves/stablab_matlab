...
% get the truncation error bound for cosin expansion: exp(bound)- (partial sum)
term = iv(0);
for k = fliplr(0:N)
...
% partial sum of Taylor expansion
out = iv(0);
for m = fliplr(0:1:N/2) % N must be even

    out = poly_add(out,(-1)^m*x{2*m+1}/fac(2*m+1));
end
